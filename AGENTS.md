...
Run `make setup` once to install dependencies, generate `.env` with worktree-aware ports, and build shared ESLint/TS configs. Bring up infrastructure with `make docker-compose-up` (PostgreSQL) and `make start-firebase-dev-emulators`. Start apps via:
...
Environment variables derive from the root `.env` (auto-generated by `make setup`). Validate changes with `bun run lint:dotenv:check` and scan for secrets using `bun run lint:secret:check`. Prisma models are verified by Lefthook pre-commit hooks—do not bypass them. Keep Firebase credentials out of version control and rely on the provided emulators during local development.
