...
describe('Admin API - /api/admin/users', () => {
  ...
  describe('GET /api/admin/users/active_users', () => {
    it('should return users by IDs', async () => {
      ...
      return request(app.getHttpServer())
        .get(`/api/admin/users/active_users?ids=${user1.body.id}&ids=${user2.body.id}`)
        .expect(200)
        .expect((res) => {
  ...
  describe('DELETE /api/admin/users/:id', () => {
    it('should soft delete a user', async () => {
      ...
      await request(app.getHttpServer()).delete(`/api/admin/users/${userId}`).expect(204);

      await request(app.getHttpServer()).get(`/api/admin/users/active_users/${userId}`).expect(404);

      await request(app.getHttpServer()).get(`/api/admin/users/deleted_users/${userId}`).expect(200);
    });
  });
  ...
  describe('PUT /api/admin/users/:id/restore', () => {
    it('should restore a soft-deleted user', async () => {
      ...
      await request(app.getHttpServer()).get(`/api/admin/users/active_users/${userId}`).expect(200);
    });
  });
  ...
  describe('DELETE /api/admin/users/hard/:id', () => {
    it('should permanently delete a user', async () => {
      ...
      await request(app.getHttpServer()).delete(`/api/admin/users/hard/${userId}`).expect(204);

      await request(app.getHttpServer()).get(`/api/admin/users/any_users/${userId}`).expect(404);
    });
  });
  ...
  describe('DELETE /api/admin/users/bulk', () => {
    it('should soft delete multiple users', async () => {
      ...
      await request(app.getHttpServer()).get(`/api/admin/users/active_users/${user1.body.id}`).expect(404);

      await request(app.getHttpServer()).get(`/api/admin/users/active_users/${user2.body.id}`).expect(404);
    });
  });
  ...
  describe('GET /api/admin/users/any_users/:id', () => {
    it('should find both active and deleted users', async () => {
      ...
      await request(app.getHttpServer()).get(`/api/admin/users/any_users/${userId}`).expect(200);

      await request(app.getHttpServer()).delete(`/api/admin/users/${userId}`).expect(204);

      await request(app.getHttpServer())
        .get(`/api/admin/users/any_users/${userId}`)
        .expect(200)
        .expect((res) => {
          expect(res.body.id).toBe(userId);
